...
- Fiz `git commit` + `git push origin main`. Segundo o `RELATORIO_SITE_UPDATE.md` anterior (2026-07-06), o projeto Pages `bora-site` era **Direct Upload** (não ligado ao Git) — ou seja, o `git push` por si só pode **não** disparar deploy automático; pode ser preciso `wrangler pages deploy` manual (conta `nilofulfarotuga@gmail.com`, já autenticada na máquina segundo o relatório anterior).
